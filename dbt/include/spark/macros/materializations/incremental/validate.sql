{% macro dbt_spark_validate_get_file_format(raw_file_format) %}
  ...
  {% set accepted_formats = ['text', 'csv', 'iceberg', 'json', 'jdbc', 'parquet', 'orc', 'hive', 'delta', 'libsvm', 'hudi'] %}
...
{% macro dbt_spark_validate_get_incremental_strategy(raw_strategy, file_format) %}
  ...
  {% set invalid_merge_msg -%}
    Invalid incremental strategy provided: {{ raw_strategy }}
    You can only choose this strategy when file_format is set to 'delta', 'iceberg', or 'hudi'
  {%- endset %}
